...
public class PermissionUtils {
    ...
    public static boolean checkPermission(Context context, String permission) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.i(TAG, "PackageInfo: " + e.toString());
        }
        //  对android版本判断
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //targetSdkVersion   判断
            Log.i(TAG, "android verison: " + Build.VERSION.SDK_INT + " targetSdkVersion" + packageInfo.applicationInfo.targetSdkVersion);
            if (packageInfo.applicationInfo.targetSdkVersion >= Build.VERSION_CODES.M) {
                Log.i(TAG, "checkpermission：" + ContextCompat.checkSelfPermission(context, permission));
            ...
            if (context.getPackageManager().checkPermission(permission, context.getPackageName()) == PackageManager.PERMISSION_GRANTED) {
                Log.i(TAG, "checkPermission: " + "拥有此权限");
                return true;
            }
            Log.i(TAG, "checkPermission: " + "没有此权限");
            return false;
    ...
    public static void checkPermission(Context context, String permission, PermissionCheckCallBack callBack) {
        if (checkPermission(context, permission)) { // 用户已授予权限
            Log.i(TAG, "用户是否授权: " + checkPermission(context, permission));
